...
export const BottomNav = {
    ...
    async render(activeTab, hiddenTabs = []) {
        ...
        nav.className = 'fixed bottom-0 w-full bg-brand-surface/70 backdrop-blur-xl border-t border-brand-border/40 safe-area-bottom z-50 shadow-[0_-4px_24px_-8px_rgba(0,0,0,0.1)] dark:shadow-[0_-4px_24px_-8px_rgba(0,0,0,0.3)]';

        const buttons = visibleTabs.map(tab => {
            const isActive = activeTab === tab.id;
            return `
                <button class="nav-item group flex flex-col items-center justify-center w-full h-full text-brand-text-secondary hover:text-brand-text-secondary transition relative active:scale-95" data-target="${tab.id}">
                    <div class="absolute top-0 w-8 h-1 bg-${tab.color} rounded-b-full transition-all duration-300 ${isActive ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'}"></div>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mt-1 mb-0.5 transition-transform group-active:scale-90 ${isActive ? `text-${tab.color}` : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ${tab.icon}
                    </svg>
        ...
        nav.innerHTML = `
            <div class="max-w-lg mx-auto h-16 pb-safe px-2">
